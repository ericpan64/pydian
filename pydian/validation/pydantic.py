...
def create_pydantic_model(model_name: str, v_map: dict[str, Rule | RuleGroup]) -> type[BaseModel]:
    ...
    for k, r_rg in v_map.items():
        rules = r_rg if isinstance(r_rg, RuleGroup) else [r_rg]
        ...
        if non_type_rules:
            ...
            field_validators[f"{k}_validator"] = field_validator(k)(dyn_field_validator)

    model = create_model(model_name, **field_definitions, __validators__=field_validators)
    return model


def _as_classmethod(rl: list[Rule | RuleGroup]) -> classmethod:
    """
    ...
    """
    return lambda cls, d: _check_err(cls, rl, d) or d
...
def _check_err(cls, rl: list[Rule | RuleGroup], rl_input: Any) -> None:
    ...
    for r in rl:
        ...
        if isinstance(res, Err):
            ...
            raise ValidationError.from_exception_data("Dynamic class validation error", errors)
    return None
